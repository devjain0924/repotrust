    ...
    <Card className="overflow-hidden">
      <CardHeader className="pb-2 flex flex-row items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">{model.provider} • v{model.version}</p>
        </div>
        <Link 
          to={`/model/${model.id}`}
      ...
      <CardContent>
        <div className="mb-4">
          ...
          <Progress 
            ...
            className={`h-2 ${
              model.trustScore >= 80 ? 'bg-success/20' : 
              model.trustScore >= 60 ? 'bg-warning/20' : 
              'bg-danger/20'
            }`}
          />
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 text-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className={model.activeFlags > 50 ? 'text-danger' : model.activeFlags > 20 ? 'text-warning' : ''}>
